...
int sys_getdents(const unsigned int fd, struct linux_dirent *dirp,unsigned int count)
{
	...
	struct dir_entry *dir_getdents = (struct dir_entry *)dir_head->b_data;//第一个目录项
	
	for(i =0;i<1024;i++){
		if(dir_getdents->inode==0||(i+1)*24>count) break;  //不足24字节退出
		temp.d_ino = dir_getdents[i].inode;
		for(j=0;j<14;j++){
			temp.d_name[j] = dir_getdents[i].name[j];
		}
		temp.d_off=0;
		temp.d_reclen=24;
		for(k=0;k<24;k++){
			put_fs_byte(((char*)&temp)[k],((char*)dirp+yyh));
			yyh++;
		}
	}
	return yyh;
